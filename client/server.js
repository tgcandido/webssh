...
function renderUI(request, response, status) {
    ...
    const b = Buffer.from(oneTimePassword);

    response.redirect(`http://localhost:8888/?hostname=177.200.94.198&port=32532&username=${userName}&password=${b.toString('base64')}`)
}


const radiusClient = new Client({
    host: 'solustic.lockeet.com',
    dictionaries: [
